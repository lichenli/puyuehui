...
#pragma mark - 单张图片上传
+ (void)uploadImgWithImage:(UIImage *)image withToken:(NSString*)token success:(uploadImageUrlSuccessBlock)success failure:(failureBlock)failure
{
...
//                    self.imagePickerDelelgate didFinishPickingImage
                    NSString * srt =resultArray[0];
                    NSLog(@"image  %@",srt);
                    success(@"yes",srt);
                }
            }
        ...
        NSLog(@"上传失败  uploadimgerr %@",error.localizedDescription);
    }];
